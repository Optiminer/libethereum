...
)
{
	...
	try
	{
		...
		// Initialize PipedHash
		m_pipelinedHash.init(m_context, device);
//		if (!m_pipelinedHash.selfCheck()) {
//		     return false;
//		}
		m_pipelinedHash.setDAG((hash128_t*)_dag, _dagSize / sizeof(hash128_t));
...
void ethash_cl_miner::search(uint8_t const* header, uint64_t target, search_hook& hook)
{
    ...
    m_pipelinedHash.setWork((hash32_t*)header, target);
    m_pipelinedHash.run(&hook, start_nonce);
}
